...
(async () => {
  ...
  // Make output buffer large enough for both (considering offset)
  const outputLength = Math.max(inputSamples.length, gptStartOffset + gptAudio.length);
  const outputSamples = new Int16Array(outputLength);
  
  // Copy input audio
  outputSamples.set(inputSamples);
  
  // Add GPT audio at the correct offset
