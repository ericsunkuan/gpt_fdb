...
(async () => {
  ...
  gptWriter.write(Buffer.from(gptAudio.buffer));
  gptWriter.end();
  ...
  // Output buffer is exactly the same length as input
  const outputSamples = new Int16Array(inputSamples.length);
  
  // Copy input audio
  outputSamples.set(inputSamples);
  
  // Add GPT audio at the correct offset, but only up to input length
  let truncated = false;
  for (let i = 0; i < gptAudio.length; i++) {
    const outputIndex = gptStartOffset + i;
    // Stop if we've reached the end of the input audio length
    if (outputIndex >= inputSamples.length) {
      truncated = true;
      break;
    }
    
    if (outputIndex >= 0) {
      const mixed = (outputSamples[outputIndex] * 0.5) + (gptAudio[i] * 0.5);
      outputSamples[outputIndex] = Math.round(mixed);
    }
  }
  
  if (truncated) {
    console.log("Note: GPT response was truncated to match input duration");
  }
  ...
  writer.write(Buffer.from(outputSamples.buffer));
  writer.end();

  // Wait for combined file to finish writing
  await new Promise((resolve) => {
    outStream.on('finish', resolve);
  });
  
  const durationSeconds = (inputSamples.length / sampleRate).toFixed(2);
  console.log(`Combined wav written to: ${outputPath} (clipped to ${durationSeconds}s)`);
